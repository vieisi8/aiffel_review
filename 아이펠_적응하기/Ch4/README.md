...
apt list --installed -> 지금까지 설치된 패키지 리스트를 확인하는 명령어
#### grep 옵션을 통해 원하는 패키지만 검색 가능

sudo apt-get update / sudo apt-get upgrade -> 패키지 업데이트(둘 다 실행하는게 좋음)

sudo apt-get remove a -> 패키지 a를 삭제
...
which a -> 프로그램 a의 설치 경로를 확인하는 명령어
